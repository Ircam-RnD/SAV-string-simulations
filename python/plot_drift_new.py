...
from fd_string_model import FD_string_model
...
for i, lambda0 in enumerate(lambda0s):
  ...
  for mode in modes:
    ...
    axs[2 * i+2].semilogy(solver.storage.t, np.abs(solver.storage.epsilon), label = mode)

  axs[2*i+1].set_ylabel(r"$f_0$ [Hz]")
  axs[2*i+1].set_ylim(80, 110)
  axs[2*i+2].set_ylabel(r"${\rm abs}(\epsilon)$")
  axs[2*i+2].set_yticks([1e-4, 1e-1])
  axs[2*i+2].set_yticklabels([1e-4, 1e-1])
...
axs[1].legend(loc = "lower center", fontsize = 8, frameon = True, fancybox = True, bbox_to_anchor = (0.5, 2.1), ncol=3)
