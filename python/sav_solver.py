...
class SAVSolver():
    ...
    def check_sizes(self):
        ...
        try:
            ...
            if (J0x.shape[0] != self.model.N or J0x.ndim != 1):
                raise Exception(
                    f"J0 x must return a vector of length {self.model.N} but has shape {J0x.shape}")
        except:
            raise Exception(
                f"J0 cannot be applied to a vector of length {self.model.N}")

        try:
            Mx = self.model.M * x
            if (Mx.shape != J0x.shape):
                raise Exception(
                    f"M x must return a vector of length {self.model.N} but has shape {Mx.shape}")
        except:
            raise Exception(
                f"M cannot be applied to a vector of length {self.model.N}")

        try:
            Rmidx = self.model.Rmid(x) * x
            if (Rmidx.shape != J0x.shape):
                raise Exception(
                    f"Rmid(x) x must return a vector of length {self.model.N} but has shape {Rmidx.shape}")
        except:
            raise Exception(
                f"Rmid(x) cannot be computed, or applied to a vector of length {self.model.N}")

        try:
            Kq = self.model.K_op(x)
        except:
            raise Exception(
                f"K_op function not working with input vector of size {self.model.N}")
        if (Kq.shape != J0x.shape):
            raise Exception(
                f"Kq must have {self.model.N} elements but has shape {Kq.shape}")
        Rsvq = self.model.Rsv_op(x)
        if (Rsvq.shape != J0x.shape):
            raise Exception(
                f"Rsvq must have {self.model.N} elements but has shape {Rsvq.shape}")
        try:
            self.Gn = self.model.G(u)
        except:
            Exception(
                f"G function not working with input vector of shape {self.u.shape}")
        if (self.Gn.shape != (self.model.N, self.model.Nu)):
            raise Exception(
                f"Gn must have shape {(self.model.N, self.model.Nu)} but has shape {self.Gn.shape}")
        try:
            self.model.Enl(x)
        except:
            raise Exception(
                f"Enl function not working with input vector of size {self.model.N}")
        try:
            Fnl_val = self.model.Fnl(x)
        except:
            raise Exception(
                f"Fnl function not working with input vector of size {self.model.N}")
        if (Fnl_val.shape != J0x.shape):
            raise Exception(
                f"Fnl(q) must have {self.model.N} elements but has shape {Fnl_val.shape}")
        try:
            Enl_val, Fnl_val = self.model.EandFnl(x)
        except:
            raise Exception(
                f"EandFnl function not working with input vector of size {self.model.N}")
        if (Fnl_val.shape != J0x.shape):
            raise Exception(
                f"Fnl(q) evaluated using EandFnl(q) must have {self.model.N} elements but has shape {Fnl_val.shape}")
    ...
    def Ek_tilde(self, p):
        ...
        return 0.5 * p.dot(1 / self.model.M * p
                           - self.dt2 / 4 * self.model.J0 / self.model.M *
                           self.model.K_op(self.model.J0 / self.model.M * p)
    ...
    def time_step(self, qlast, qnow, rn, unow, ConstantRmid=False, BoundG=False):
    ...
    def integrate(self, q0, u0, u_func, duration, ConstantRmid=False,
                  storage_config=DEFAULT_STORAGE_CONFIG, plotter_config=DEFAULT_PLOTTER_CONFIG, BoundG=False):
        ...
        try:
            for i in range(self.Nt):
                qnext, rnext, qn, pn, epsilon = self.time_step(
                    qlast, qnow, rnow, u_func((i+0.5) * self.dt), ConstantRmid=ConstantRmid, BoundG=BoundG)
    ...
    def time_step_verlet(self, qlast, qnow, unow, ConstantRmid=False):
        ...
        self.RHSn = self.B_op(qnow) + self.C_op_Verlet(qlast, self.Rmidn) + \
            self.Gn @ unow - self.model.J0 * self.model.Fnl(qnow)
    ...
    def integrate_verlet(self, q0, u0, u_func, duration, ConstantRmid=False,
        ...
        try:
            for i in range(self.Nt):
                qnext, qn, pn = self.time_step_verlet(qlast, qnow, u_func(
                    (i+0.5) * self.dt), ConstantRmid=ConstantRmid)
