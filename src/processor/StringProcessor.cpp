...
{
  T0 = sqrt((pow(M_PI, 4) * E * pow(f0, 2) * pow(R, 6) * rho) /
            (beta * (1 + beta)));
...
void StringProcessor<T>::setDissFromDecays()
{
  ...
  eta_0 = fmax(3 * log(10) / (beta2_1 - beta2_0) *
                   (beta2_1 / t60_0 - beta2_0 / t60_1),
               0);
...
{
  return (-gamma2 + sqrt(gamma2 * gamma2 + 4 * kappa2 * omega * omega)) /
         (2 * kappa2);
}
...
void StringProcessor<T>::modifyhFromBend()
{
  ...
  // T lbend = 1/(2*fbend) * sqrt(T0/mu) * sqrt(1 + pow(M_PI, 3) * E * pow(R, 4)
  // / (4 * l0 * l0));
  T a = fbend * fbend;
  T b = -T0 / (4 * mu);
  ...
  h = fmax(lbend / N,
           sqrt((gamma + sqrt(gamma * gamma + 16 * mu * E * I * pow(dt, 2))) /
                (2 * mu)));
}
...
void StringProcessor<T>::reinitDsp(float sampleRate)
{
  ...
  dt = 1 / sr;
  T gamma = dt * dt * T0 + 4 * dt * mu * eta_1;
  h = sqrt((gamma + sqrt(gamma * gamma + 16 * mu * E * I * pow(dt, 2))) /
           (2 * mu));
...
void StringProcessor<T>::updateCoefficients()
{
  ...
  D42 = 1 / (pow(h, 4));
  Current0 =
      (2 * mu - 2 * pow(dt / h, 2) * T0 - 4 * mu * eta_1 * dt / pow(h, 2)) *
          Eigen::Vector<T, -1>::Ones(N - 1) -
      pow(dt, 2) * E * I * D40;
  Last0 = -(1 - dt * eta_0) * mu + 4 * mu * dt * eta_1 / pow(h, 2);
  Current1 = pow(dt / h, 2) * T0 - pow(dt, 2) * E * I * D41 +
...
void StringProcessor<T>::computeVAndVprime()
{
  ...
  case 4:
    ...
    V = (E * A - T0) / 8 * h * (dxq3.cwiseProduct(dxq)).sum() +
        ((-qnow + boundary).cwiseMax(0).array().pow(alphac + 1)).sum() * kc /
            (alphac + 1) * h;
    Vprime =
        -(E * A - T0) / 2 * (dxq3.tail(N - 1) - dxq3.head(N - 1)) -
        ((-qnow + boundary).cwiseMax(0).array().pow(alphac)).matrix() * kc * h;
    break;
  default:
    V = 0;
...
void StringProcessor<T>::computeV()
{
  ...
  case 2:
    ...
    break;
  case 3:
    V = ((-(qnow + qlast) / 2 + boundary).cwiseMax(0).array().pow(alphac + 1))
            .sum() *
        kc / (alphac + 1) * h;
    break;
  case 4:
    ...
    V = (E * A - T0) / 8 * h * (dxq3.cwiseProduct(dxq)).sum() +
        ((-(qnow + qlast) / 2 + boundary).cwiseMax(0).array().pow(alphac + 1))
                .sum() *
            kc / (alphac + 1) * h;
    break;
  default:
...
template <class T>
std::tuple<T, T, T> StringProcessor<T>::process(T input, T bend, T posex,
                                                T poslistL, T poslistR,
                                                T t60_0)
{
  ...
  if (controlTerm)
  {
    ...
    g += -lambda0 * epsilon * dt *
         ((qnow - qlast).array() > 0)
             .select(Eigen::Vector<T, -1>::Ones(N - 1),
                     -Eigen::Vector<T, -1>::Ones(N - 1)) /
         ((qnow - qlast).template lpNorm<1>() + 1e-12);
  }

  // Linear part
  righthand = Current0.cwiseProduct(qnow) + Last0 * qlast;
  righthand.head(N - 2) +=
      Current1 * qnow.tail(N - 2) + Last1 * qlast.tail(N - 2);
  righthand.tail(N - 2) +=
      Current1 * qnow.head(N - 2) + Last1 * qlast.head(N - 2);

  righthand.tail(N - 3) += Current2 * qnow.head(N - 3);
  righthand.head(N - 3) += Current2 * qnow.tail(N - 3);

  // External force
  righthand(static_cast<int>(floor(this->posex * (N - 2)))) +=
      pow(dt, 2) * input / h *
      (1 - (this->posex * (N - 2) - floor(this->posex * (N - 2))));
  righthand(static_cast<int>(ceil(this->posex * (N - 2)))) +=
      pow(dt, 2) * input / h *
      ((this->posex * (N - 2) - floor(this->posex * (N - 2))));
...
template <class T>
std::tuple<T, T, T> StringProcessor<T>::processBowed(T vbow, T Fbow, T bend,
                                                     T posex, T poslistL,
                                                     T poslistR)
{
  vbow *= 1e-5;
  ...
  vrel = (qnow(static_cast<int>(posex * (N - 2))) -
          qlast(static_cast<int>(poslistL * (N - 2)))) /
             dt -
         vbow;
  // Compute bow force
  phinow = phi(vrel);
  Rbow.setZero();
  Rbow(static_cast<int>(posex * (N - 2))) =
      abs(Fbow) * phinow / (vrel + std::copysign(1e-12, vrel));
  ...
  // Linear part
  righthand = Current0.cwiseProduct(qnow) +
              (Last0 * Eigen::Vector<T, -1>::Ones(N - 1) + dt * Rbow * h * 0.5)
                  .cwiseProduct(qlast) -
              Rbow * vrel;
  righthand.head(N - 2) +=
      Current1 * qnow.tail(N - 2) + Last1 * qlast.tail(N - 2);
  ...
  // External force
  // righthand(static_cast<int>(floor(this->posex*(N-2)))) += pow(dt, 2) * input
  // / h * (1 - (this->posex*(N-2) - floor(this->posex*(N-2))));
  // righthand(static_cast<int>(ceil(this->posex*(N-2)))) += pow(dt, 2) * input
  // / h * ((this->posex*(N-2) - floor(this->posex*(N-2))));
  ...
  // Solving using shermann morrisson
  term0V = (mu * ((1 + dt * eta_0) * Eigen::Vector<T, -1>::Ones(N - 1) +
                  dt * Rbow * h * 0.5))
               .cwiseInverse();
  qnext = term0V.cwiseProduct(righthand) -
          pow(dt / 2, 2) * 1 / h * term0V.cwiseProduct(term0V).cwiseProduct(g) *
              g.dot(righthand) /
              (1 + pow(dt / 2, 2) * 1 / h * term0V.cwiseProduct(g).dot(g));
...
{
  vl = ((qnow(static_cast<int>(floor(poslistL * (N - 2)))) -
         qlast(static_cast<int>(floor(poslistL * (N - 2))))) *
            (1 - (poslistL * (N - 2) - floor(poslistL * (N - 2)))) +
        (qnow(static_cast<int>(ceil(poslistL * (N - 2)))) -
         qlast(static_cast<int>(ceil(poslistL * (N - 2))))) *
            (poslistL * (N - 2) - floor(poslistL * (N - 2)))) /
       (dt * sqrt(T0 * mu));
  vr = ((qnow(static_cast<int>(floor(poslistR * (N - 2)))) -
         qlast(static_cast<int>(floor(poslistR * (N - 2))))) *
            (1 - (poslistR * (N - 2) - floor(poslistR * (N - 2)))) +
        (qnow(static_cast<int>(ceil(poslistR * (N - 2)))) -
         qlast(static_cast<int>(ceil(poslistR * (N - 2))))) *
            (poslistR * (N - 2) - floor(poslistR * (N - 2)))) /
       (dt * sqrt(T0 * mu));
}
